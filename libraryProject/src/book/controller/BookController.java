...
public class BookController extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		//공통 부분인 paging.jsp를 위해서 path를 조회해서 넘겨준다.
		...
		if(action.equals("/bookList.do")) {
			...
			if(b_search.equals("all") || b_search==null) {
				b_search = "b_title||b_name";
				request.setAttribute("b_search", "all");
			}else {
				request.setAttribute("b_search", b_search);
			}
			
			String q = request.getParameter("q");
			request.setAttribute("q", q);
			
			command.booksSearch(request, response, b_search, q, paging);
			nextPage = "/views/books/booksList.jsp";
